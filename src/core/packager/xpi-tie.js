goog.provide('ckft.xpiTie');

/**
 * This method assembles all of the required files to write the xpi file into a
 * temporary directory, then writes the contents of this temporary directory to
 * an xpi file. The temporary directory is a folder with a unique,
 * auto-generated name in the same directory that the xpi file will be written
 * to. This temporary directory is deleted upon completion of the packaging.
 * The xpi file and the chickenfoot-xpi-tie.jar inside of it are then zipped
 * using the nsIZipWriter XPCOM component.
 *
 * @param {string} outputPath full absolute path of where the xpi file will be
 *     written to
 * @param {Object} templateTags map of strings that be used to fill in template
 *     files
 * @param {Array.<Trigger>} triggers array of the Trigger objects to be included
 *     in the xpi file
 * @param {Array.<string>} userFiles array of other filepaths (not triggers) to
 *     be included in the xpi file
 * @param {string} iconPath full absolute path of the icon for the extension, or
 *     null to use the default icon
 * @param {ChromeWindow} chromeWindow reference to chromeWindow of browser
 * @return {nsIFile} object of generated xpi file
 */
ckft.xpiTie.createXpiTie = function(outputPath,
                                    templateTags,
                                    triggers,
                                    userFiles,
                                    iconPath,
  ...
  // Create a temporary directory inside the system's designated temporary
  // directory for building the jar and xpi files.
  ...
  // get extension path
  var mgr = Components.classes["@mozilla.org/extensions/manager;1"].
  ...
  ckft.xpiTie.writeXpiTieJar(tempDir, extFile, templateTags, iconPath, chromeWindow);
  
  // assemble contents for and zip xpi file
  ckft.xpiTie.writeXpiFile(tempDir, tempDirPath, extFile, outputPath, templateTags, triggers, userFiles, chromeWindow);

  return xpiFile;
};

/**
 * This inner function first assembles all the needed files for the
 * chickenfoot-xpi-tie.jar file into a 'content' directory inside the temporary
 * directory. Then it writes the chickenfoot-xpi-tie jar to a 'chrome' directory
 * inside the temporary directory.
 * @param {nsILocalFile} tempDir
 * @param {nsILocalFile} extFile
 * @param {Object} templateTags
 * @param {string} iconPath
 * @param {ChromeWindow} chromeWindow reference to chromeWindow of browser
 */
ckft.xpiTie.writeXpiTieJar = function(tempDir, extFile, templateTags, iconPath, chromeWindow) {
  var io = SimpleIO;

  // create chickenfoot-xpi-tie.jar
  var contentDir = tempDir.clone();
  contentDir.append("chickenfoot-xpi-tie_TEMP_DIR");
  ...
  if ((iconPath != null) && (iconPath != "") &&
      io.exists(io.toFile(iconPath))) { //get user's image if supplied 
    ...
    // this init call is needed in Firefox 2 and lower, but throws an exception
    // in Firefox 3, so just catch it and continue
  ...
  ckft.xpiTie.writeToZip(chromeWindow, xpiTempDir.path, xpiTieFile.path);
...
/**
 * This inner function first assembles all the needed files for the xpi file
 * into the temporary directory. Then it writes all the files inside the
 * temporary directory to the xpi file, and deletes the temporary directory.
 * @param {nsILocalFile} tempDir
 ...
 * @param {ChromeWindow} chromeWindow reference to chromeWindow of browser
 */
ckft.xpiTie.writeXpiFile = function(tempDir, tempDirPath, extFile, outputPath, templateTags, triggers, userFiles, chromeWindow) {
  var io = SimpleIO;

  // COMPONENTS DIRECTORY is a copy of Chickenfoot's components directory
  // _except for_ ChickenfootCommandLineHandler.js.
  ...
  if (io.exists(commandLineHandlerFile)) {
    commandLineHandlerFile.remove(false);
  }
  ...
  var triggersXmlTxt = ckft.xpiTie.createTriggersXml(triggers);
  ...
  var tempDir = Components.classes["@mozilla.org/file/local;1"].
      createInstance(Components.interfaces.nsILocalFile);
  tempDir.initWithPath(tempDirPath);
  ckft.xpiTie.writeToZip(chromeWindow, tempDirPath, outputPath);
  tempDir.remove(true);
...
/**
 * This method takes a string and replaces all instances of the keys of the map
 * with the value that they are mapped to. Just a replace-all function on the
 * string for the keys of the map with their values.
 *
 * @param {string} txt is the string that will be serve as the template
 * @param {Object} map is keys and values for the template
 * @return the modified string
 */
function fillTemplate(txt, map) {
  for (var property in map) {
    txt = txt.replace(new RegExp("@" + property + "@", "g"), map[property]);
  }
  return txt;
};

/**
 * This method writes all the files inside the temporary directory to a new zip
 * file located at the output file path.
 *
 * @param {ChromeWindow} chromeWindow reference to chromeWindow of browser
 * @param {string} tempDirPath full absolute path of the temporary directory
 * @param {string} outputPath full absolute path where the zip file will be written to
 **/
ckft.xpiTie.writeToZip = function(chromeWindow, tempDirPath, outputPath) {
  ...
  while (tempDirFiles.hasMoreElements()) {
    var fileToWrite = tempDirFiles.getNext().QueryInterface(
        Components.interfaces.nsIFile); 
    ckft.xpiTie.writeFileToZip(fileToWrite, zipWriter, null /* dirName */, chromeWindow);
  }
  zipWriter.close();
};

/**
  * This method writes a single file or directory to a zip file using the given
  * zip writer. For directories, this method recursively calls itself on each of
  * the files in the directory. The file or directory is not modified or
  * deleted, only copied. This method never uses a reference to the actual zip
  * file or its path, it only uses the given zip writer.
  * 
  * @param {!nsIFile} currentFile object to be written to the zip file
  * @param {!nsIZipWriter} zipWriter used to write to the zip file
  * @param {string} dirName optional prefix to attach to the file name in the
  *     zip file, ignored if null
  * @param {ChromeWindow} chromeWindow reference to chromeWindow of browser
  */
ckft.xpiTie.writeFileToZip = function(currentFile, zipWriter, dirName, chromeWindow) {
  if (currentFile.isFile()) {
    ...
    try {
      zipWriter.addEntryFile(fileName,
                             Components.interfaces.nsIZipWriter.COMPRESSION_DEFAULT,
                             currentFile,
                             false);
    } catch(e) {
      chromeWindow.alert('caught exception : nsIZipWriter');
    }  
  } else {
    ...
    while (listFiles.hasMoreElements()) {
      var fileToWrite = listFiles.getNext().QueryInterface(Components.interfaces.nsIFile); 
      ckft.xpiTie.writeFileToZip(fileToWrite, zipWriter, subDirName, chromeWindow);
    }
  }
};

/**
 * Creates a triggers.xml file (as a string) given a list of triggers.
 * @param {Array.<Trigger>} triggers to add to the triggers.xml file
 * @return {string} the generated xml document as a string
 */
ckft.xpiTie.createTriggersXml = function(triggers) {
  ...
  for (var i = 0; i < triggers.length; i++) {
    // Fixed bug #382: when triggers are packaged as an extension,
    // all triggers should be enabled in the package
    triggers[i].enabled = true;
    gTriggerManager._appendTriggerXmlNode(xmlDoc, docElement, triggers[i]);
  }
  var buffer = [];
  gTriggerManager._prettyPrint(xmlDoc, "", buffer);
  return buffer.join('');
};
